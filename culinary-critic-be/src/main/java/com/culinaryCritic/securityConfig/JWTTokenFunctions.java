...
import com.culinaryCritic.securityConfig.filter.Constants.SecurityConstants;
...
public class JWTTokenFunctions {
    public String generateToken(Authentication user) {
        ...
        return Jwts.builder().setIssuer("culinaryCritic").setSubject("JWT Token").claim("username", user.getName()).claim("role", role).setIssuedAt(new Date()).setExpiration(new Date((new Date().getTime() + 30000000))).signWith(key).compact();
